...
namespace ManejoPresupuesto.Servicios
{
    ...
    public class RepositorioTiposCuentas : IRepositorioTiposCuentas
    {
        ...
        public RepositorioTiposCuentas(IConfiguration contiguration) {
            connectionString = contiguration.GetConnectionString("DefaultConnection");
        }
        ...
        public async Task<bool> Existe(string nombre, int usuarioId, int id = 0) {
            using var connection = new SqlConnection(connectionString);
            var existe = await connection.QueryFirstOrDefaultAsync<int>(@"SELECT 1 FROM TiposCuentas WHERE Nombre = @Nombre AND UsuarioId = @UsuarioId AND Id <> @id;",
